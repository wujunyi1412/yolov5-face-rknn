...
#include <set>
#include <vector>
#define LABEL_NALE_TXT_PATH "./model/face_labels_list.txt"
        ...
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                ...
                if (box_confidence >= thres_u8)
                {
                    ...
                    {
                        uint8_t prob = in_ptr[(15 + k) * grid_len];
...
static int process_i8(int8_t *input, int *anchor, int grid_h, int grid_w, int height, int width, int stride,
                      std::vector<float> &boxes, std::vector<float> &objProbs, std::vector<int> &classId, float threshold,
                      int32_t zp, float scale, std::vector<float> &landmarks)
{
    int validCount = 0;
    int grid_len = grid_h * grid_w;
        ...
        for (int i = 0; i < grid_h; i++)
        {
            for (int j = 0; j < grid_w; j++)
            {
                ...
                if (box_confidence >= thres_i8)
                {
                    ...
                    float lambmark_x1 = (deqnt_affine_to_f32(in_ptr[5 * grid_len], zp, scale));
                    float lambmark_y1 = (deqnt_affine_to_f32(in_ptr[6 * grid_len], zp, scale));
                    float lambmark_x2 = (deqnt_affine_to_f32(in_ptr[7 * grid_len], zp, scale));
                    float lambmark_y2 = (deqnt_affine_to_f32(in_ptr[8 * grid_len], zp, scale));
                    float lambmark_x3 = (deqnt_affine_to_f32(in_ptr[9 * grid_len], zp, scale));
                    float lambmark_y3 = (deqnt_affine_to_f32(in_ptr[10 * grid_len], zp, scale));
                    float lambmark_x4 = (deqnt_affine_to_f32(in_ptr[11 * grid_len], zp, scale));
                    float lambmark_y4 = (deqnt_affine_to_f32(in_ptr[12 * grid_len], zp, scale));
                    float lambmark_x5 = (deqnt_affine_to_f32(in_ptr[13 * grid_len], zp, scale));
                    float lambmark_y5 = (deqnt_affine_to_f32(in_ptr[14 * grid_len], zp, scale));
                    ...
                    lambmark_x1 = lambmark_x1 * (float)anchor[a * 2] + j * (float)stride;
                    lambmark_y1 = lambmark_y1 * (float)anchor[a * 2 + 1] + i * (float)stride;
                    lambmark_x2 = lambmark_x2 * (float)anchor[a * 2] + j * (float)stride;
                    lambmark_y2 = lambmark_y2 * (float)anchor[a * 2 + 1] + i * (float)stride;
                    lambmark_x3 = lambmark_x3 * (float)anchor[a * 2] + j * (float)stride;
                    lambmark_y3 = lambmark_y3 * (float)anchor[a * 2 + 1] + i * (float)stride;   
                    lambmark_x4 = lambmark_x4 * (float)anchor[a * 2] + j * (float)stride;
                    lambmark_y4 = lambmark_y4 * (float)anchor[a * 2 + 1] + i * (float)stride;   
                    lambmark_x5 = lambmark_x5 * (float)anchor[a * 2] + j * (float)stride;
                    lambmark_y5 = lambmark_y5 * (float)anchor[a * 2 + 1] + i * (float)stride;

                    int8_t maxClassProbs = in_ptr[15 * grid_len];
                    int maxClassId = 0;
                    for (int k = 1; k < OBJ_CLASS_NUM; ++k)
                    {
                        int8_t prob = in_ptr[(15 + k) * grid_len];
                        if (prob > maxClassProbs)
                        {
                            maxClassId = k;
                            maxClassProbs = prob;
                    ...
                    if (limit_score >= threshold)
                    {
                        ...
                        boxes.push_back(box_w);
                        boxes.push_back(box_h);
                        landmarks.push_back(lambmark_x1);
                        landmarks.push_back(lambmark_y1);
                        landmarks.push_back(lambmark_x2);
                        landmarks.push_back(lambmark_y2);   
                        landmarks.push_back(lambmark_x3);
                        landmarks.push_back(lambmark_y3);   
                        landmarks.push_back(lambmark_x4);
                        landmarks.push_back(lambmark_y4);   
                        landmarks.push_back(lambmark_x5);
                        landmarks.push_back(lambmark_y5);
                    }
                }
            }
        }
...
int post_process(rknn_app_context_t *app_ctx, void *outputs, letterbox_t *letter_box, float conf_threshold, float nms_threshold, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (app_ctx->is_quant)
        {
            validCount += process_i8((int8_t *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
                                     classId, conf_threshold, app_ctx->output_attrs[i].zp, app_ctx->output_attrs[i].scale, landmarks);
        }
        else
        {
            validCount += process_fp32((float *)_outputs[i].buf, (int *)anchor[i], grid_h, grid_w, model_in_h, model_in_w, stride, filterBoxes, objProbs,
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        float landmarks_x1 = landmarks[n * 10 + 0] - letter_box->x_pad;
        float landmarks_y1 = landmarks[n * 10 + 1] - letter_box->y_pad;
        float landmarks_x2 = landmarks[n * 10 + 2] - letter_box->x_pad;
        float landmarks_y2 = landmarks[n * 10 + 3] - letter_box->y_pad;
        float landmarks_x3 = landmarks[n * 10 + 4] - letter_box->x_pad;
        float landmarks_y3 = landmarks[n * 10 + 5] - letter_box->y_pad;
        float landmarks_x4 = landmarks[n * 10 + 6] - letter_box->x_pad;
        float landmarks_y4 = landmarks[n * 10 + 7] - letter_box->y_pad;
        float landmarks_x5 = landmarks[n * 10 + 8] - letter_box->x_pad;
        float landmarks_y5 = landmarks[n * 10 + 9] - letter_box->y_pad;
        ...
        od_results->results[last_count].landmarks.x1 = (int)(clamp(landmarks_x1, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].landmarks.y1 = (int)(clamp(landmarks_y1, 0, model_in_h) / letter_box->scale);
        od_results->results[last_count].landmarks.x2 = (int)(clamp(landmarks_x2, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].landmarks.y2 = (int)(clamp(landmarks_y2, 0, model_in_h) / letter_box->scale);
        od_results->results[last_count].landmarks.x3 = (int)(clamp(landmarks_x3, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].landmarks.y3 = (int)(clamp(landmarks_y3, 0, model_in_h) / letter_box->scale);
        od_results->results[last_count].landmarks.x4 = (int)(clamp(landmarks_x4, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].landmarks.y4 = (int)(clamp(landmarks_y4, 0, model_in_h) / letter_box->scale);
        od_results->results[last_count].landmarks.x5 = (int)(clamp(landmarks_x5, 0, model_in_w) / letter_box->scale);
        od_results->results[last_count].landmarks.y5 = (int)(clamp(landmarks_y5, 0, model_in_h) / letter_box->scale);
